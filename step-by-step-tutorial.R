...
# This is an EXTENDED tutorial example of how to implement the method discribed by Olave et al. (2017; Methods in Ecology and Evolution)
  # This tutorial has a clear explanation of each step of the pipeline. But, I have written a new short tutorial = quick.tutorial.R. This quick script implements a function that automatise all the process described here.
...
# The sequence of steps from (i) to (iii) is shown in (Fig. 2; Olave et al. 2017 - MEE) and corresponds to:
...
wd <- getwd(); # getting root directory
...
# read the sp tree as text, to later use it at input to get.topo2ms function
...
# NOTE: always check if the topology is in the right format. For this case, it should look like this: "-ej 1 2 1 -ej 2 3 1 -ej 3 4 1"
topology
...
# define H
nloci <- 100; # number of loci or gene trees that want to simulate (H). Because this tutorial is ment as example, and not real analyses, then only set nloci = 100. Recommended for a real analyses = 10,000

# name your ms output file (will contain all simulated gene trees for expectations). It is usually recommended to add as much details as possible of the simulated parameters: number of individuals per species and M parameter (in this M = 0);
...
PhylonetDir <- file.path(wd, "phylonet_v2_4.jar"); # other version more than phylonet_v2_4.jar might not work!
...
distVec <- sptree.vs.genetrees(wd=wd, PhylonetDir=PhylonetDir, tree.newick=tree.newick, 
                               genetreeNames=Exp.outputName, species=species, Imap=Imap, sp.treeName="sp-tree.tre");
...
setwd(wd);
likelihood <- getLikelihood(genetreeFile = genetreeFiles,
                            genetree.Path="genetrees",
                            distVec=distVec, 
                            sp.treeName = "sp-tree.tre", 
...
#you can save the progres:
write.table(result, "results.txt", sep="\t", col.names=T);

# and later you can load it again:
result <- read.table("results.txt", sep="\t", header=T);

# NOTE: the steps so far need to be repeated with a different model, including some magnitudes of M, for example try:
# When having M different to 0, you can use gf.code2ms to automathically convert the format
  # time = time when gene flow occurred back in time (then 0 means current gene flow);
  # species1 = from
  # species2 = recieving species
  # M= migration parameter magnitude
...
new.results <- data.frame(distVecName, "M=0", likelihood);
colnames(new.results) <- c("distVec.Source", "M", "likelihood");
result <- rbind(result, new.results);
result

# Lets say that these are all the models that we are interested to test, thus you can perform a likelihood ratio test between this two models
